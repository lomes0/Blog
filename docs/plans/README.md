...
| [mcp_support.md](./mcp_support.md)                         | **Phases 1–3 shipped (8 Aug 2026)** — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. `POST /api/mcp` works end to end: a `tokenRoute` wrapper, scopes gated at tool registration, and two tokens on one server proved to see two different authors' content. **Not fit for the internet until phase 4** — the app still has no rate limiting anywhere |
