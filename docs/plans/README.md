...
| [byo-provider-keys.md](./byo-provider-keys.md)             | **Phases 1–4 shipped 15 Aug 2026**, phase 5 (rotation) open. AI provider keys move from the deployment's `.env` to a per-user encrypted row, with no fallback. Introduces the codebase's first reversible secret, which is why §4.2 (AAD, IV, key versioning) and §6 (what this does *not* protect) carry most of the weight. Live: completions run on the user's own key and the deployment's `*_API_KEY` variables are deleted, so one set there serves nobody |
