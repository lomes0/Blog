...
| [blob-storage.md](./blob-storage.md)                       | **Phases 1–3 done for images (15 Aug 2026); 4–5 open** — one content-addressed store for every byte, on R2. Measured before designing, and the migration bore it out: one PNG stored 67 times took the dev database from 34 MB to 19 MB. §3.1, §3.2 and §10.1 are corrections written while building. Sketches and graphs are held back on one rendering decision (§10.1); phase 4 is now urgent rather than optional, because docx export throws on a blob URL. Supersedes [archive/storage-uploads.md](./archive/storage-uploads.md) |
