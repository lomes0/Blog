...
| [mcp_support.md](./mcp_support.md)                         | **Phases 1–4 shipped (8 Aug 2026)** — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. `POST /api/mcp` works end to end: a `tokenRoute` wrapper, scopes gated at tool registration, two tokens on one server proved to see two different authors' content, and three token-bucket budgets (requests → 429, reads and writes → tool error). Brought the app its first rate limiter, `src/lib/rateLimit.ts`, written generic for `/api/completion` next. Phase 5 still owes it TLS enforcement |
