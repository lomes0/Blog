...
| [mcp_support.md](./mcp_support.md)                         | **Phase 1 shipped (8 Aug 2026)** — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. The server is a factory now (`src/lib/mcp/server.ts`) and `mcp/` finally has specs; still to come are the token model, a fourth `tokenRoute` wrapper (§4.2), and rate limiting the app has none of |
