...
| [blob-storage.md](./blob-storage.md)                       | **Phase 1 done, phase 2 done bar sketches (15 Aug 2026); phases 3–5 open** — one content-addressed store for every byte, on R2. Measured before designing: six distinct images are stored 141 times, one PNG is a third of the database, ~25× reclaimable. §3.1 and §3.2 are corrections from building reconciliation; the next step is phase 3, the migration that reclaims the 13 MB. Supersedes [archive/storage-uploads.md](./archive/storage-uploads.md) |
