...
All four of the items that were blocked on a human answer rather than on work
have one, and the last two closed on 31 Aug 2026. **Nothing on this page is
waiting on a human.**
...
- **Nested editors: address into them, or refuse explicitly?** — **address into
  ...
  rather than nesting.
- **Local drafts the terminal cannot see: close the gap or accept it?** —
  **accept**, taken 31 Aug 2026 as `claude-code-backlog.md` §7. The two agents
  read different libraries because of where each runs, not by oversight, and
  both ways of closing it are worse than the gap: syncing guest drafts to the
  server is a product change with its own questions, and giving the MCP server a
  browser is absurd. The "and document" half of its own recommendation was
  already discharged — the content guide leads its caveats section with it.
