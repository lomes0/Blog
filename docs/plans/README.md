...
| [mcp_support.md](./mcp_support.md)                         | **Phases 1–2 shipped (8 Aug 2026)** — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. The server is a factory (`src/lib/mcp/server.ts`), `mcp/` finally has specs, and `AgentToken` + `npm run mcp:token` mint/list/revoke many tokens per user. Still to come: a fourth `tokenRoute` wrapper (§4.2), the endpoint itself, and rate limiting the app has none of |
