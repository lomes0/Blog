...
| [mcp_support.md](./mcp_support.md)                         | **Phases 1–5 shipped (8 Aug 2026); only phase 6 (docs) remains** — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. `POST /api/mcp` works end to end: a `tokenRoute` wrapper, scopes gated at tool registration, two tokens on one server proved to see two different authors' content, three token-bucket budgets, 426 on cleartext, and an origin that names which token wrote ("Claude Code (laptop)"). Brought the app its first rate limiter, `src/lib/rateLimit.ts`, written generic for `/api/completion` next |
