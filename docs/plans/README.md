...
| [mcp_support.md](./mcp_support.md)                         | Proposal — the eight MCP tools over HTTP from any machine, authenticated by a per-user bearer token. Bearer not OAuth (§4.1), a fourth `tokenRoute` wrapper (§4.2), and rate limiting the app has none of. Measured against an ssh one-liner that already works                             |
