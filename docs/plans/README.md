...
| [claude-code-backlog.md](./claude-code-backlog.md)         | **Backlog.** What the content bridge does _not_ do, and why. **Nothing on it is capability work any more (27 Aug 2026)** — §4 was answered "address into them" and `nested-editor-support.md` shipped it, which closed §2 as well: a canvas's notes became addressable rather than merely describable. What is left is four decisions — §3 (semantic search, needs a concrete failure first), §5 (how loudly a proposal should name a rich block it deletes), §6 (series placement on create) and §7 (local drafts, recommendation: accept and document) |
...
| [workspace-url.md](./workspace-url.md)                     | **Phase A shipped 28 Aug 2026**, phases B–D open — the workspace URL should stop projecting pane focus and become an entry point. Refines [archive/workspace-panes.md](./archive/workspace-panes.md) §0 rather than reversing it. Phase A severed the six places that re-derived the open document from `window.location.pathname`, which fixes attachment upload on a handle URL by construction; §8.1 records what had drifted under the plan by the time it ran, chiefly that §4 named three of those six |
