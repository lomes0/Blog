...
| [ordering-simplification.md](./ordering-simplification.md) | **Phases 1-3 shipped 30 Aug 2026** — four order arrays (the plan's §2 misses `Project`), backfilled from `rank` and verified identical across all 54 containers, and every read switched to them through one tolerant reader. `rank` still owns writes and is dual-written into the arrays, which is the half phase 4 deletes. §11 is the phase log; it lists the five other places the plan had gone stale |
