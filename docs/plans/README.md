...
| [blob-storage.md](./blob-storage.md)                       | **Decided 13 Aug 2026, not started** — one content-addressed store for every byte, on R2. Measured before designing: six distinct images are stored 141 times, one PNG is a third of the database, ~25× reclaimable. Supersedes [archive/storage-uploads.md](./archive/storage-uploads.md) |
| [byo-provider-keys.md](./byo-provider-keys.md)             | **COMPLETE 15 Aug 2026** — all five phases. AI provider keys move from the deployment's `.env` to a per-user encrypted row, with no fallback. Introduces the codebase's first reversible secret, which is why §4.2 (AAD, IV, key versioning) and §6 (what this does *not* protect) carry most of the weight. Live: completions run on the user's own key, the `*_API_KEY` variables are deleted so one set there serves nobody, and `pnpm ai:rotate` replaces the KEK without downtime (verified by SIGKILLing it mid-rotation over 2 500 rows) |
