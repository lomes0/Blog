...
| [byo-provider-keys.md](./byo-provider-keys.md)             | **Proposal, 15 Aug 2026, not started** — AI provider keys move from the deployment's `.env` to a per-user encrypted row, with no fallback. Introduces the codebase's first reversible secret, which is why §4.2 (AAD, IV, key versioning) and §6 (what this does *not* protect) carry most of the weight |
