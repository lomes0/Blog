...
| [byo-provider-keys.md](./byo-provider-keys.md)             | **Phases 1–3 shipped 15 Aug 2026**, phases 4–5 open. AI provider keys move from the deployment's `.env` to a per-user encrypted row, with no fallback. Introduces the codebase's first reversible secret, which is why §4.2 (AAD, IV, key versioning) and §6 (what this does *not* protect) carry most of the weight. Storage, route, CLI and settings UI are in; completions still run on the deployment's env keys, because the switch is deliberately the *last* phase |
