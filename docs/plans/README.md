...
`ide-redesign.md` followed it on 2 Sep 2026, and closed the other way — by
**deleting** its last two items rather than building them. Both specified their
target by reference to a proposal bundle that had left the repo, so neither
could say what it wanted, and a month on this table did not change that. A row
that reads "blocked on respecification" is worth re-reading as "nobody wants
this enough to specify it", which is a decision available immediately rather
than a task waiting on one. It is in
[archive/](./archive/ide-redesign.md) now.
