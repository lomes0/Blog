...
const TutorialEditor: React.FC<PropsWithChildren> = (
  ...
  const onChange = (
    ...
    editor.update(() => {
      ...
      const root = $getRoot();
      const testSuites = root.getChildren().filter(
        $isDetailsContainerNode,
      );
      for (let i = 0; i < testSuites.length; i++) {
        const testSuite = testSuites[i];
        if (!testSuite.isParentOf(selectedNode)) continue;
        const content = testSuite.getChildren().find(
          $isDetailsContentNode,
        );
        if (!content) continue;
        const tasks = content.getChildren().filter(
          $isDetailsContainerNode,
        );
        for (let j = 0; j < tasks.length; j++) {
          ...
          const nextSiblings = nextSibling.getNodesBetween(
            tasks[j + 1] ?? testSuites[i + 1] ?? root,
          );
          if (
            !nextSiblings.some((node) =>
              node.is(selectedNode) ||
              node.isParentOf(selectedNode)
            )
          ) continue;
          const checkList = task.getChildren()?.find(
            $isDetailsSummaryNode,
          )?.getFirstChild();
