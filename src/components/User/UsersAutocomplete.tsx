...
export default function UsersAutocomplete({
  ...
  const userValue = value.map((u) =>
    typeof u === "string" ? users.find((user) => user.email === u) || u : u
  );
  const handleChange = (
    event: React.SyntheticEvent,
    newValue: (User | string)[],
  ) => {
    const invalidEmails = newValue.filter((u) =>
      typeof u === "string" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(u)
    );
    ...
    <Autocomplete
      ...
      renderTags={(tagValue, getTagProps) => {
        return tagValue.map((option, index) => {
          ...
          const image = typeof option === "string"
            ? undefined
          ...
          return (
            <Chip
              ...
              key={email}
              label={name}
              avatar={<Avatar alt={altText} src={image} />}
            />
          );
        });
      }}
