...
const authorSeries = async (authorId: string) => {
  ...
  // (docs/plans/archive/ordering-simplification.md §2). A series inside a
  // project is not in that array and so falls to the end by `createdAt`, which
  // is the tolerant reader's rule (§6). A flat list of every series has no
  // single order to be in: a project's members are ordered by that project, not
  // against the root list, so there is nothing better to sort them by here.
