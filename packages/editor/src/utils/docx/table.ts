...
export function $convertTableNode(node: TableNode) {
  ...
  const float = $getNodeStyleValueForProperty(node, "float").replace(
    "none",
    "",
  );
  const alignment = (node.getFormatType().replace("justify", "both") ||
    "both") as IParagraphOptions["alignment"];
  const columnWidth = (float || alignment !== "both")
    ? 75 * 15
    : 600 * 15 / columnCount;
  const columnWidths = node.getColWidths()?.map((width) =>
  ...
  const table = new Table({
    ...
    width: (float || alignment !== "both")
      ? undefined
      : { size: 100, type: "pct" },
    columnWidths: columnWidths
      ? columnWidths.map((width) => width * 15)
      : Array(columnCount).fill(columnWidth),
  ...
  if (!id) {
    return [
      table,
      new Paragraph({
        spacing: { before: float ? 0 : 8 * 15, after: 0, line: 0 },
      }),
    ];
  }
...
function $convertTableRowNode(node: TableRowNode) {
  ...
  const isHeader = cellNodes.some((cell) =>
...
function $convertTableCellNode(node: TableCellNode) {
  ...
  const color =
    $getNodeStyleValueForProperty(node, "color").replace("inherit", "") ||
    undefined;
  ...
  const backgroundColor = $getNodeStyleValueForProperty(
    ...
    "background-color",
    isHeader ? "#f5f5f5" : "",
  ).replace("inherit", "") || undefined;
  const float = $getNodeStyleValueForProperty(TableNode, "float").replace(
    "none",
    "",
  );
  const alignment = TableNode.getFormatType().replace(
    "justify",
    "both",
  ...
  return new TableCell({
    ...
    rowSpan: rowSpan,
    width: cellWidth
      ? { size: cellWidth, type: "dxa" }
      : { size: 100 / cellCount, type: "pct" },
