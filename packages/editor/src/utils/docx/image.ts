...
export function $convertImageNode(node: ImageNode) {
  ...
  const nearesttLayoutContainer = $findMatchingParent(
    node,
    $isLayoutContainerNode,
  );
  const layoutTemplate = nearesttLayoutContainer?.getTemplateColumns().split(
    " ",
  ).map(parseFloat);
  const LayoutItemNodeIndex = $findMatchingParent(node, $isLayoutItemNode)
    ?.getIndexWithinParent();
  const maxLayoutItemWidth = layoutTemplate
    ? 600 * layoutTemplate[LayoutItemNodeIndex || 0] /
      layoutTemplate.reduce((a, b) => a + b, 0)
    : 600;
  ...
  const imageRun = new ImageRun({
    ...
    floating: float && !showCaption
      ? {
        horizontalPosition: {
          relative: "margin",
          align: float === "left" ? "left" : "right",
        },
        verticalPosition: { relative: "paragraph", align: "top" },
        allowOverlap: false,
        wrap: {
          type: TextWrappingType.SQUARE,
          side: float === "left" ? "right" : "left",
        },
        margins: {
          top: 0,
          bottom: 0,
          left: float === "right" ? 100720 : 0,
          right: float === "left" ? 100720 : 0,
        },
      }
      : undefined,
  });
  ...
  const captionChildren = showCaption
    ? caption.getEditorState().read($convertEditortoDocx)
    : [];
  const id = node.getId();
  ...
  if (!showCaption) {
    return [
      ...
      new TextRun({ text: "", break: 1, vanish: !showCaption }),
      ...captionChildren,
    ];
  }
  const parent = node.getParent() as ParagraphNode;
  const alignment = parent.getFormatType().replace(
    "justify",
    "both",
...
function svgToBuffer(svg: string) {
  const html = decodeURIComponent(svg).replace(
