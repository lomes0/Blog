...
render path is permanent even so**: a guest's documents keep their data URIs
(§11.1), so `SketchNode` and `GraphNode` both branch on the `src` prefix and
must keep doing it. Collection is `pnpm blobs:collect`
